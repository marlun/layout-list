...
module.exports = function moveActiveVertically (list, direction) {
  assert(Array.isArray(list), 'list should be type Array')
  assert(['down', 'up'].includes(direction), 'invalid direction')
  ...
  function findTarget (parent, child) {
    ...
    // If parent direction is "row" we need to go further up the tree
  ...
  function findTargetInContainer (parent, current) {
    ...
    if (currentParent.direction === 'column' && parent.direction === 'row') {
      target = children[0]
      if (target.direction !== undefined) {
        target = findTargetInContainer(target, current)
      }
